...
namespace MALM.UI
{
	internal partial class MikrotikAddressTableRecord_AddItemUI : Form
	{
		...
		private Dictionary<string, ListViewGroupCollapsedState> _groups;
		private string _selectedGroupName;
		private SuggestionItemBase[] _addressSuggestionList;
		...
		private void OnLoad()
		{
...

			{
				...
				cboAddress.SelectedIndexChanged += delegate
				{
					...
					if (mko != null)
					{
						cboAddress.eRunDelayedInUIThread(delegate
						{
...

			{
				...
				var ll = _addressSuggestionList
					.Select(si =>
					{
						...
						li.Group = lvwSources!.eGroupsCreateGroupByKey(si.GrpupName, newGroupState: gs).Group;

						return li;
					})
					.ToArray();

				lvwSources.Items.AddRange(ll);
				lvwSources.AutoResizeColumns(ColumnHeaderAutoResizeStyle.ColumnContent);

				lvwSources.SelectedIndexChanged += (_, _) =>
				{
					var sel = lvwSources.eSelectedItemsAs<uom.controls.ListViewItemTRO<SuggestionItemBase>>()
						.FirstOrDefault();
