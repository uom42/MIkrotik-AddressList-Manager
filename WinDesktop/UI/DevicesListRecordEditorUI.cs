...
internal partial class DevicesListRecordEditorUI : Form
{
    ...
    private DevicesListRecordEditorUI () : base()
    {
        ...
        txtPort.SetCueBanner(L_LEAVE_EMPTY_FOR_DEFAULT);
    ...
    private bool ValidateUserInput ()
    {
        try
        {
            ...
            if ( !string.IsNullOrWhiteSpace(txtPort.Text.Trim()) && !ushort.TryParse(txtPort.Text.Trim() , out ushort iPort) ) throw new ArgumentOutOfRangeException(L_DEVICE_PORT);
            return string.IsNullOrWhiteSpace(txtUser.Text.Trim()) ? throw new ArgumentNullException(L_DEVICE_USER) : true;
        }
        catch { }
        return false;
    }
