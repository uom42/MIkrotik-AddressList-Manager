	...
	internal class MasterKeyManager
	{
		...
		private async Task LoadKey ()
		{
			static async Task<(string?, bool)> GetMasterKey ()
			{
				...
				bool dontAskKey = loadedKeyString.IsNotNullOrWhiteSpace();
#if DEBUG
				...
				static string? DecryptMasterKey ( IsolatedStorageFile isf )
				{
					...
					try
					{
						...
						return masterKeyBytes.eToStringUni ();
					}
...
#endif
		{
			...
			if (userKey.IsNullOrWhiteSpace ()) throw new ArgumentNullException ("MasterKey");
		...
		private static async Task WriteOrDeleteMK ( string? masterKey )
		{
			...
			//else
			{
				if (masterKey.IsNullOrWhiteSpace ()) throw new ArgumentNullException ("MasterKey");
				//Saving Masterkey
