...
namespace MALM.Model
{
    ...
    internal class LocalLANHost //: Mikrotik.API.Model.IP.ARP.ARPListItem
    {
        ...
        private static readonly Lazy<OUI.OUIDatabase?> _ouiDb = new(() => OUI.Manager.Database);
        ...
        private readonly Color? _clrError = null;
        ...
        internal LocalLANHost ( MikrotikDotNet.Model.IP.DHCPServer.LeaseItem dhcp ) : this(dhcp.ActiveMacAddress! , [ dhcp.ActiveAddress! ] , SOURCES.DHCP)
        {
        ...
        internal LocalLANHost (
            ...
            : this(arp.MacAddress! , [ arp.Address! ] , SOURCES.ARP)
        {
        ...
        public void Update ( ListViewItem li , ListView lvw )
        {
...
#if WINDOWS
            if ( LastSeen.HasValue && Math.Truncate(LastSeen.Value.TotalMilliseconds) < uint.MaxValue )
            {
            ...
            string ipString = (IPList.Length == 1)
                ...
                : $"({IPList.Length}): " + IPList.Select(ip => ip.ToString()).join(", ")!;
...
#if WINDOWS
            li.Group = lvw.groupsCreateGroupByKey(
            Source.ToString() ,
