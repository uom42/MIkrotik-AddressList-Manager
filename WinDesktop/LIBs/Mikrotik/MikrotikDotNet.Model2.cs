...
namespace MikrotikDotNet.Model
{
    ...
    internal static class Helpers
    {
        ...
        public const ushort DEFAULT_API_PORT = MikrotikDotNet.MKConnection.DEFAULT_API_PORT;


        [DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static MKConnection CreateConnection ( string address , string user , string pwd , ushort port = DEFAULT_API_PORT )
        {
            ...
            return pwd.isNullOrWhiteSpace ? throw new ArgumentNullException(nameof(pwd)) : new(address , user , pwd , port);
        }


        [DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static async Task<MKConnection> ConnectDeviceAsync ( string address , string user , string pwd , ushort port = DEFAULT_API_PORT , int timeout = DEVICE_CONNECT_TIMEOUT_DEFAULT )
        {
        ...
        public static async Task<MKConnection> ConnectDeviceAsync ( MKConnection con , int timeout = DEVICE_CONNECT_TIMEOUT_DEFAULT )
        {
            ...
            if ( await Task.WhenAny(tConnect , Task.Delay(timeout , ct.Token)) == tConnect )
            {
                ...
                await tConnect;
                return !con.IsOpen ? throw new Exception(E_MIKROTIK_CONNECTION_FAILED) : con;
            }
            else
            {
    ...
    {
        Task EnableAsync ( bool useOpenedConnection , bool enable );
    }
    ...
    namespace System
    {
        ...
        public class DeviceStatus
        {
			...
			*/
            ...
            public ulong FreeMemory { get; private set; } = 0;
            public ulong TotalMemory { get; private set; } = 0;


            public ulong FreeHddSpace { get; private set; } = 0;
            public ulong TotalHddSpace { get; private set; } = 0;
            public ulong WriteSectSinceReboot { get; private set; } = 0;
            public ulong WriteSectTotal { get; private set; } = 0;
            public ulong? BadBlocks { get; private set; }
            ...
            public DateTime? BuildTime_Parsed
            {
                get
                {
                    return DateTime.TryParse(BuildTime , MikrotikDotNet.MKHelpers.CultureEnUs.Value , out var dt) ? dt : null;
                }
            }
            ...
            public string BuildTime_Friendly
            {
                get
                {
                    try
                    {
                        ...
                        return BuildTime_Parsed == null || !BuildTime_Parsed.HasValue
                            ? string.Empty
                            : BuildTime_Parsed.Value.ToString(dtfi.MonthDayPattern + " yyyy");
                    }
                    catch { }
			...
			public string FreeMemory_String => FreeMemory.eFormatByteSizeMP();
			public string TotalMemory_String => TotalMemory.eFormatByteSizeMP();

			public float FreeHddSpace_Percent => (float)((double)FreeHddSpace / (double)TotalHddSpace);
			public string FreeHddSpace_String => FreeHddSpace.eFormatByteSizeMP();
			public string TotalHddSpace_String => TotalHddSpace.eFormatByteSizeMP();
			...
			public uom.InvertableBool ErrorStste_BadBlocks => (BadBlocks != null && BadBlocks.HasValue && BadBlocks.Value > 0ul);

			public uom.InvertableBool ErrorStste_HighTemp => (Temperature > 65);

			public uom.InvertableBool ErrorStste_HighCPUUsage => (CpuLoad > 90u);

			public uom.InvertableBool ErrorStste_LowVoltage => (Voltage < 12.0f);

			public uom.InvertableBool ErrorStste_LowRAM => (FreeMemory_Percent < .1f);

			public uom.InvertableBool ErrorStste_LowHddSpace => (FreeHddSpace_Percent < .1f);

			public uom.InvertableBool ErrorStste_FWtooOld
            ...
            [DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static async Task<DeviceStatus> GetDeviceStatusAsync ( string address , string user , string pwd , ushort port = Helpers.DEFAULT_API_PORT , int timeout = 3_000 )
            {
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
                ...
                /// <summary>Refresh item info from Mikrotik by item ID</summary>
                public static async Task<AddressListItem?> Add (
                    MKConnection mc ,
                    bool useOpenedConnection ,
