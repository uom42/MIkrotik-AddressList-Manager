...
namespace MikrotikDotNet.Model
{
	...
	internal static class Extensions
	{
		...
		[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uom.Network.IP4AddressWithMask? e_ParseIPMasked(this string? ipm)
		{
			try
			{
				if (ipm.e_IsNotNullOrWhiteSpace())
	...
	namespace System
	{
		...
		public class DeviceStatus
		{
			...
			*/
			...
			public string FreeMemory_String => FreeMemory.e_FormatByteSize();
			public string TotalMemory_String => TotalMemory.e_FormatByteSize();

			public float FreeHddSpace_Percent => (float)((double)FreeHddSpace / (double)TotalHddSpace);
			public string FreeHddSpace_String => FreeHddSpace.e_FormatByteSize();
			public string TotalHddSpace_String => TotalHddSpace.e_FormatByteSize();
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
					...
					string? comment = null)
				{
					...
					return await GetItemsAsync(mc, condition, MKQueryLogicOperators.And);
				}
				...
				{
					AddressListItem? newItem = (await GetItemsAsync(Connection, id: MKID))
				...
				static public async Task<AddressListItem?> Add(MKConnection mc, string list, string address, bool disabled, string comment = "")
				{
					...
					AddressListItem? existingItem = (await GetItemsAsync(mc, address: address, list: list, disabled: disabled))
						.FirstOrDefault();
					...
					cmd.Parameters.Add(FIELD_DISABLED, disabled.ToDeviceBoolString());
				...
				public async Task EnableAsync(bool enable)
					=> await SetPropertyAsync(COMMAND_LIST_SET, FIELD_DISABLED, (!enable).ToDeviceBoolString());
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
				...
				public static async Task<IEnumerable<ARPItem>> GetItemsAsync(MKConnection mc)
				{
					var cmd = mc!.CreateCommand(COMMAND_LIST_PRINT);
					cmd.Parameters.Add(COMMAND_PARAM_DETAIL, string.Empty);

					return Sort(await GetItemsAsync<ARPItem>(mc, cmd));
				}
				...
				public Task EnableAsync(bool enable) => throw new NotImplementedException();
		...
		namespace DHCPServer
		{
			...
			public class LeaseItem : MKDataRowBase, IEnableItem
			{
				...
				//internal IPAddress? Address_Parsed => Address.e_ParseIP();
				//internal PhysicalAddress? MacAddress_Parsed => MacAddress.e_ParseMAC();
				//internal IPAddress? ActiveAddress_Parsed => ActiveAddress.e_ParseIP();
				//internal PhysicalAddress? ActiveMacAddress_Parsed => ActiveMacAddress.e_ParseMAC();
				//internal TimeSpan? ExpiresAfter_Parsed => ExpiresAfter.e_ParseTimeSpan();
				//internal TimeSpan? LastSeen_Parsed => LastSeen.e_ParseTimeSpan();

				//internal STATUSES Status_Parsed => Status.e_ToEnumValue(STATUSES.unknown);
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
				...
				[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
				public static async Task<IEnumerable<LeaseItem>> GetItemsAsync(MKConnection mc)
					=> Sort(
				...
				[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
				public static async Task<IEnumerable<LeaseItem>> GetItemsAsync(MKConnection mc, MKCommandParameterCollection condition, MKQueryLogicOperators logic)
					=> Sort(
					 await GetItemsAsync<LeaseItem>(
						 mc,
						 mc.CreateCommand(COMMAND_LIST_PRINT),
	...
	*/
				...
				internal uom.Network.IP4AddressWithMask? Address_Parsed => Address.e_ParseIPMasked();
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
				...
				[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
				public static async Task<IEnumerable<AddressItem>> GetItemsAsync(MKConnection mc)
					=> Sort(
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
				...
				[DebuggerNonUserCode, DebuggerStepThrough, MethodImpl(MethodImplOptions.AggressiveInlining)]
				public static async Task<IEnumerable<BridgeItem>> GetItemsAsync(MKConnection mc)
					=> Sort(
