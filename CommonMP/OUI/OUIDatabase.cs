...
\t{4}(?<ADDR_COUNTRY>.+)\r\n";
		...
		Dictionary<string, MacRecordsGroup> _fastCache = [];
		...
		internal static OUIDatabase? TryCreate()
		{
			...
			{
				//Loading
				body = fiBase.e_ReadAsText(detectEncodingFromByteOrderMarks: true)!;
		...
		public static async Task UpdateDatabase(FileInfo localDBTargetPath)
		{
			...
			fiTemp.e_DeleteIfExistSafe();

			try
			{
				FileInfo? f = await uom.Network.Helpers.DownloadFile_HttpClient_Async(REMOTE_OIU_DATABASE, tempFile, downloadTimeout, ct);
				if (f == null || !f.Exists || (UInt64)f.Length < 3u.e_MBToBytes())
				{
					f?.e_DeleteIfExistSafe();
					return;//Failed to download
				}
				localDBTargetPath.Refresh();
				localDBTargetPath.e_DeleteIfExist();
				//if (fiDatabaseTarget.Exists) fiDatabaseTarget.e_MoveToArhive().e_ClearArhiveOldFiles(DateTime.Now.AddMonths(-2));
...
#if WINDOWS
				exweb.e_LogError(false);				
#else
				exweb.e_LogErrorNoUI();
#endif
			}
			catch (Exception ex)
			{
			...
			using (WebClient WC = new())
			{
				...
				try
				{
					...
					WC.DownloadProgressChanged += (sender, e) =>
						{
							...
							string sProgress = "{0} - Downloading file {1}% from {2} to {3}".e_Format(TMR.Elapsed.ToString(), iProgress, C_REMOTE_OIU_FILE, tempFile);
							Debug.WriteLine(sProgress);
						};

					Debug.WriteLine("Start downloading file from {0} to {1}".e_Format(C_REMOTE_OIU_FILE, tempFile));
					...
					bool bDownloaded = await evtDownloadFinished.e_WaitAsync(iTimeout: iWaitSec);
					if (!bDownloaded)
					{
						bCanceled = true;
						if (WC.IsBusy) WC.CancelAsync();

						var toex = new TimeoutException("Не удалось загрузить '{0}' за {1}".e_Format(C_REMOTE_OIU_FILE, ((int)TMR.ElapsedMilliseconds).e_ToShellTimeString(5)));
						throw toex;
					}
					...
					// Downloaded ok.
					Debug.WriteLine("Download finished at {0}".e_Format(TMR.Elapsed.ToString()));
					...
					ulong lOUISizeBytes_Min = C_DATABASE_SIZE_MIN_MB.e_MBToBytes();
					ulong lOUISizeBytes_Max = C_DATABASE_SIZE_MAX_MB.e_MBToBytes();
					...
					if (fiTemp.Length < (decimal)lOUISizeBytes_Min || fiTemp.Length > (decimal)lOUISizeBytes_Max)
					{
						...
						throw new Exception(C_ERROR.e_Format(C_REMOTE_OIU_FILE, tempFile, fiTemp.Length.e_FormatByteSize(), lOUISizeBytes_Min.e_FormatByteSize(), lOUISizeBytes_Max.e_FormatByteSize()).e_WrapVB());
					}

					// Move to Release
					C_LOCAL_OUI_DATABASE.e_MakeBackUpIfExist(true);
					Debug.WriteLine("Move file {0} to {1}".e_Format(tempFile, C_LOCAL_OUI_DATABASE));
		...
		public bool GetMacRecordString(PhysicalAddress mac, out string? mfgString, out MacRecordsGroup? mrg)
		{
			...
			mfgString = (mriList.Length == 1)
							? mriList[0].ToString()
							: mriList.Select(r => r.ToString()).e_Join(" ,") ?? string.Empty;
